...
@Component
public class Co2LevelConditionChecker extends AnalyzerConditionChecker {
    public boolean check(Condition condition, SensorStateAvro state) {
        ClimateSensorAvro sensor = (ClimateSensorAvro) state.getData();
        return isOperationTrue(condition.getOperation(), condition.getValue(), sensor.getCo2Level());
    }
